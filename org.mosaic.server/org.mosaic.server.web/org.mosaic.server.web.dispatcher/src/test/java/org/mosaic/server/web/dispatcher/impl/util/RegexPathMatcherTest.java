...
public class RegexPathMatcherTest extends Assert {
    ...
    private void testPattern( String pattern, String path, boolean shouldMatch, Variable... vars ) {
        ...
        String desc = "Matching of pattern '" + pattern + "' (regex is '" + result.getRegex() + "') with input '" + path + "' ";

        assertEquals(
                desc + ( shouldMatch ? "should succeed" : "should NOT succeed" ),
                shouldMatch,
                result.isMatching() );
        ...
        for( Variable var : vars ) {
            if( !matchedVars.containsKey( var.name ) ) {
                ...
                assertEquals( desc + " wrong value for var '" + var.name + "'", var.value, matchedVars.get( var.name ) );
            }
        }
