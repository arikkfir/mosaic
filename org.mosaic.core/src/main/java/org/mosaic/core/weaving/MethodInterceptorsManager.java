...
{
    @Nonnull
    private static final Logger LOG = LoggerFactory.getLogger( MethodInterceptorsManager.class );
    ...
    @Nullable
    private ServiceListenerRegistration<MethodInterceptor> interceptorListenerRegistration;

    public MethodInterceptorsManager( @Nonnull ServerImpl server )
    {
        this.lock = server.getLock();

        server.addStartupHook( bundleContext -> WeavingSpi.setMethodInterceptorsManager( this ) );
        server.addShutdownHook( bundleContext -> WeavingSpi.setMethodInterceptorsManager( null ) );
        ...
        server.addStartupHook( bundleContext -> {
            ...
            this.interceptorListenerRegistration =
                    server.getServiceManager().addListener( null,
                                                            this.methodInterceptorServiceListener,
                                                            MethodInterceptor.class );
        } );
        server.addShutdownHook( bundleContext -> {
            ServiceListenerRegistration<MethodInterceptor> registration = this.interceptorListenerRegistration;
        ...
        return this.lock.read( () -> {
            ...
            else
            {
                return this.lock.write( () -> {
                    ...
                    List<InterestedMethodInterceptorEntry> interceptors2 = methodInterceptorsForMethods.get( method );
                    if( interceptors2 == null )
                    {
                        ...
                        for( MethodInterceptor methodInterceptor : this.methodInterceptors )
                        {
                            ...
                            if( methodInterceptor.interestedIn( method, context ) )
                            {
                                if( interceptors2 == null )
                                {
                                    interceptors2 = new LinkedList<>();
                                }
                                interceptors2.add( new InterestedMethodInterceptorEntry( methodInterceptor, new HashMap<>( context ) ) );
                            }
                        }
                        if( interceptors2 == null )
                        {
                            interceptors2 = Collections.emptyList();
                        }
                        methodInterceptorsForMethods.put( method, interceptors2 );
                    }
