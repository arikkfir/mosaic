...
{
    void weaveNonnullChecks( @Nonnull WovenClass wovenClass, @Nonnull CtClass ctClass )
    {
        for( CtBehavior behavior : ctClass.getDeclaredBehaviors() )
        {
            try
            {
                    ...
                    for( Object annotationObject : availableParameterAnnotations[ i ] )
                    {
                        ...
                        if( annotation.annotationType().equals( Nonnull.class ) )
                        {
                            behavior.insertBefore(
                                    BytecodeUtil.javaCode( "if( $%d == null )                                                                                                     \n" +
                                                           "{                                                                                                                     \n" +
                ...
                if( behavior instanceof CtMethod )
                {
                    ...
                    {
                        method.insertAfter( BytecodeUtil.javaCode(
                                "if( $_ == null )                                                                                       \n" +
                                "{                                                                                                      \n" +
            ...
            {
                throw new WeavingException( "could not weave @Nonnull for behavior '" + behavior.getLongName() + "' of '" + ctClass.getName() + "' in " + BytecodeUtil.toString( wovenClass.getBundleWiring().getRevision() ), e );
            }
        }
    }
}
