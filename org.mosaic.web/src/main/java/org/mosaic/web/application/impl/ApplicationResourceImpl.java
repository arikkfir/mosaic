 ...
 * @author arik
 */
final class ApplicationResourceImpl implements Application.ApplicationResource
{
    ...
    ApplicationResourceImpl( @Nonnull Path contentRoot, @Nonnull Path path )
    {
        this.path = path;
            ...
            if( exists( propertiesFile ) )
            {
                try( Reader reader = newBufferedReader( propertiesFile, Charset.forName( "UTF-8" ) ) )
                {
                    ...
                    if( cachePeriod == null )
                    {
                        ...
                        {
                            cachePeriod = ApplicationManager.parsePeriod( cachePeriodValue );
                        }
                    }
                }
                catch( Throwable e )
